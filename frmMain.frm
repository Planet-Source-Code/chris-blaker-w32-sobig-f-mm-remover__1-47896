...
Begin VB.Form frmMain 
   ...
   Caption         =   "W32.SOBIG.F@MM - Remover"
   ClientHeight    =   3262
   ClientLeft      =   42
   ClientTop       =   364
   ClientWidth     =   5474
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "w32.sobig.f@mm remover for Windows."
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   8
         Charset         =   0
...
Private Sub cmdBegin_Click()
    ...
    CreateNewKey "", hKey
    SetKeyValue HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "sobig remover", Addtoreg, REG_SZ  ' Add the program to the registry so it will perform it's second task after reboot
    DeleteValue HKEY_LOCAL_MACHINE, "\SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "TrayX"  ' Remove the virus from the registry.
    DeleteValue HKEY_CURRENT_USER, "\SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "TrayX"  ' Remove the virus from the registry.
...
If cmdBegin.Tag = "1" Then  ' If the application has already done the first part, proceed to the next.
    ...
    Kill "C:\WINDOWS\winppr32.exe"  '----| Remove Virus from XP/9x/ME
    
    Kill "C:\WINNT\winppr32.exe"    '----| Remove Virus from NT/2k


    
    DeleteValue HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "sobig remover"
